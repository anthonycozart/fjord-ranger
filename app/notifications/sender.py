...
Sends a notification SMS to a user about a newly available slot.
On success: creates Notification + UserSlotState rows, increments
the user's daily counter, and commits.
On failure: rolls back all DB changes and logs the error.

Call notify_user() via asyncio.to_thread — the Twilio SDK is synchronous.
"""
...
def _format_sms(session: MomenceSession, slot_code: str) -> str:
    ...
    return (
        ...
        f"Reply NO-{slot_code} to skip. STOP to unsubscribe."
    )
...
def notify_user(user: User, session: MomenceSession, db: Session) -> bool:
    ...
    try:
        ...
        # Twilio confirmed — record SID and commit everything
